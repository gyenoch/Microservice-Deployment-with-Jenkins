resource "aws_security_group" "Server_sg" {
  ...
  ingress = [
    for port in [22, 80, 443, 465, 8080, 9000, 9200, 9090, 5601, 5044] : {
...
resource "aws_iam_role_policy_attachment" "microservice_role_attach" {
...
resource "aws_iam_instance_profile" "microservice_instance_profile" {
  name = "microservice-instance-profile"
  role = aws_iam_role.microservice_role.name
}
