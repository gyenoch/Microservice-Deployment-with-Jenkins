pipeline {
    ...
    stages {
            ...
            steps {
                script {
                    withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker') {
                        sh "docker build -t gyenoch/adservice:${BUILD_NUMBER} ."
                    }
                }
            }
        }
        ...
        stage('Docker Image Scan') {
            steps {
                sh 'trivy image gyenoch/adservice:${BUILD_NUMBER} >> trivyimage.txt'
                script {
        ...
        stage('Push Docker Image') {
            steps {
                script {
                    withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker') {
                        sh "docker push gyenoch/adservice:${BUILD_NUMBER} "
                    }
                }
            }
        }
            ...
            steps {
                withCredentials([string(credentialsId: 'github', variable: 'GITHUB_APP')]) {
                    sh '''
                        ...
                        imageTag=$(grep 'image:\\s*gyenoch/adservice:' ${DEPLOYMENT_FILE} | sed -E 's/.*gyenoch\\/adservice:([a-zA-Z0-9._-]+)/\\1/')
                        if [ -z "$imageTag" ]; then
                        ...
                        sed -i "s|gyenoch/adservice:$imageTag|gyenoch/adservice:$BUILD_NUMBER|" ${DEPLOYMENT_FILE}
...
//                 script {
//                     withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker') {
//                         sh "docker build -t kanha05/adservice:latest ."
//                     }
//                 }
//             }
//         }
...
//                 script {
//                     withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker') {
//                         sh "docker push kanha05/adservice:latest "
//                     }
//                 }
//             }
//         }
