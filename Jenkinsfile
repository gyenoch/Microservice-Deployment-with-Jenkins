pipeline {
    ...
    stages {
            ...
            steps {
                script {
                    withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker') {
                        sh "docker build -t gyenoch/emailservice:${BUILD_NUMBER} ."
                    }
                }
            }
        }
        ...
        stage('Docker Image Scan') {
            steps {
                sh 'trivy image gyenoch/emailservice:${BUILD_NUMBER} >> trivyimage.txt'
                script {
                    def scanResults = readFile('trivyimage.txt')
                    // Log the scan results without throwing an error
                    echo "Emailservice scan results:\n${scanResults}"
                }
            }
        }
        ...
        stage('Push Docker Image') {
            steps {
                script {
                    withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker') {
                        sh "docker push gyenoch/emailservice:${BUILD_NUMBER} "
                    }
                }
            }
        }
            ...
            steps {
                withCredentials([string(credentialsId: 'github', variable: 'GITHUB_APP')]) {
                    sh '''
                        ...
                        imageTag=$(grep 'image:\\s*gyenoch/emailservice:' ${DEPLOYMENT_FILE} | sed -E 's/.*gyenoch\\/emailservice:([a-zA-Z0-9._-]+)/\\1/')
                        if [ -z "$imageTag" ]; then
                        ...
                        sed -i "s|gyenoch/emailservice:$imageTag|gyenoch/emailservice:$BUILD_NUMBER|" ${DEPLOYMENT_FILE}
...
//                 script {
//                     withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker') {
//                         sh "docker build -t gyenoch/emailservice:latest ."
//                     }
//                 }
//             }
//         }
...
//                 script {
//                     withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker') {
//                         sh "docker push gyenoch/emailservice:latest "
//                     }
//                 }
//             }
//         }
