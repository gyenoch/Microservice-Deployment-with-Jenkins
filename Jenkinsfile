pipeline {
    ...
    stages {
                ...
                script {
                    dir('src') {
                        withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker') {
                            sh "docker build -t gyenoch/cartservice:${BUILD_NUMBER} ."
                        }
                    }
                }
            }
        ...
        stage('Docker Image Scan') {
            steps {
                sh 'trivy image gyenoch/cartservice:${BUILD_NUMBER} >> trivyimage.txt'
                script {
        ...
        stage('Push Docker Image') {
            steps {
                script {
                    withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker') {
                        sh "docker push gyenoch/cartservice:${BUILD_NUMBER} "
                    }
                }
            }
        }
            ...
            steps {
                withCredentials([string(credentialsId: 'github', variable: 'GITHUB_APP')]) {
                    sh '''
                        ...
                        imageTag=$(grep 'image:\\s*gyenoch/cartservice:' ${DEPLOYMENT_FILE} | sed -E 's/.*gyenoch\\/cartservice:([a-zA-Z0-9._-]+)/\\1/')
                        if [ -z "$imageTag" ]; then
                        ...
                        sed -i "s|gyenoch/cartservice:$imageTag|gyenoch/cartservice:$BUILD_NUMBER|" ${DEPLOYMENT_FILE}
