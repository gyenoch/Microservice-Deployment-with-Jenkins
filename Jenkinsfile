pipeline {
    ...
    stages {
            ...
            steps {
                script {
                    withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker') {
                        sh "docker build -t gyenoch/checkoutservice:${BUILD_NUMBER} ."
                    }
                }
            }
        }
        ...
        stage('Docker Image Scan') {
            steps {
                sh 'trivy image gyenoch/checkoutservice:${BUILD_NUMBER} >> trivyimage.txt'
                script {
                    def scanResults = readFile('trivyimage.txt')
                    // Log the scan results without throwing an error
                    echo "Checkoutservice scan results:\n${scanResults}"
                }
            }
        }
        ...
        stage('Push Docker Image') {
            steps {
                script {
                    withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker') {
                        sh "docker push gyenoch/checkoutservice:${BUILD_NUMBER} "
                    }
                }
            }
        }
            ...
            steps {
                withCredentials([string(credentialsId: 'github', variable: 'GITHUB_APP')]) {
                    sh '''
                        ...
                        imageTag=$(grep 'image:\\s*gyenoch/checkoutservice:' ${DEPLOYMENT_FILE} | sed -E 's/.*gyenoch\\/checkoutservice:([a-zA-Z0-9._-]+)/\\1/')
                        if [ -z "$imageTag" ]; then
                        ...
                        sed -i "s|gyenoch/checkoutservice:$imageTag|gyenoch/checkoutservice:$BUILD_NUMBER|" ${DEPLOYMENT_FILE}
...
//                 script {
//                     withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker') {
//                         sh "docker build -t gyenoch/checkoutservice:latest ."
//                     }
//                 }
//             }
//         }
...
//                 script {
//                     withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker') {
//                         sh "docker push gyenoch/checkoutservice:latest "
//                     }
//                 }
//             }
//         }
