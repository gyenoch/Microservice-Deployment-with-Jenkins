pipeline {
    ...
    stages {
        stage('Checkout') {
            steps {
                checkout([$class: 'GitSCM',
                    branches: [[name: '*/productcatalogservice']],
            ...
            steps {
                script {
                    withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker') {
                        sh "docker build -t gyenoch/productcatalogservice:${BUILD_NUMBER} ."
                    }
                }
            }
        }
        ...
        stage('Docker Image Scan') {
            steps {
                sh 'trivy image gyenoch/productcatalogservice:${BUILD_NUMBER} >> trivyimage.txt'
                script {
                    def scanResults = readFile('trivyimage.txt')
                    // Log the scan results without throwing an error
                    echo "productcatalogservice scan results:\n${scanResults}"
                }
            }
        }
        ...
        stage('Push Docker Image') {
            steps {
                script {
                    withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker') {
                        sh "docker push gyenoch/productcatalogservice:${BUILD_NUMBER} "
                    }
                }
            }
        }
            ...
            steps {
                withCredentials([string(credentialsId: 'github', variable: 'GITHUB_APP')]) {
                    sh '''
                        ...
                        imageTag=$(grep 'image:\\s*gyenoch/productcatalogservice:' ${DEPLOYMENT_FILE} | sed -E 's/.*gyenoch\\/productcatalogservice:([a-zA-Z0-9._-]+)/\\1/')
                        if [ -z "$imageTag" ]; then
                        ...
                        sed -i "s|gyenoch/productcatalogservice:$imageTag|gyenoch/productcatalogservice:$BUILD_NUMBER|" ${DEPLOYMENT_FILE}
...
//                 script {
//                     withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker') {
//                         sh "docker build -t kanha05/productcatalogservice:latest ."
//                     }
//                 }
//             }
//         }
...
//                 script {
//                     withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker') {
//                         sh "docker push kanha05/productcatalogservice:latest "
//                     }
//                 }
//             }
//         }
