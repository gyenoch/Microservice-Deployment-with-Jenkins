pipeline {
    ...
    stages {
            ...
            steps {
                script {
                    withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker') {
                        sh "docker build -t gyenoch/loadgenerator:${BUILD_NUMBER} ."
                    }
                }
            }
        }
        ...
        stage('Docker Image Scan') {
            steps {
                sh 'trivy image gyenoch/loadgenerator:${BUILD_NUMBER} >> trivyimage.txt'
                script {
                    def scanResults = readFile('trivyimage.txt')
                    // Log the scan results without throwing an error
                    echo "Loadgenerator scan results:\n${scanResults}"
                }
            }
        }
        ...
        stage('Push Docker Image') {
            steps {
                script {
                    withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker') {
                        sh "docker push gyenoch/loadgenerator:${BUILD_NUMBER} "
                    }
                }
            }
        }
            ...
            steps {
                withCredentials([string(credentialsId: 'github', variable: 'GITHUB_APP')]) {
                    sh '''
                        ...
                        imageTag=$(grep 'image:\\s*gyenoch/loadgenerator:' ${DEPLOYMENT_FILE} | sed -E 's/.*gyenoch\\/loadgenerator:([a-zA-Z0-9._-]+)/\\1/')
                        if [ -z "$imageTag" ]; then
                        ...
                        sed -i "s|gyenoch/loadgenerator:$imageTag|gyenoch/loadgenerator:$BUILD_NUMBER|" ${DEPLOYMENT_FILE}
...
//                 script {
//                     withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker') {
//                         sh "docker build -t gyenoch/loadgenerator:latest ."
//                     }
//                 }
//             }
//         }
...
//                 script {
//                     withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker') {
//                         sh "docker push gyenoch/loadgenerator:latest"
//                     }
//                 }
//             }
//         }
